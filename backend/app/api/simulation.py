...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        if not entity:
            return jsonify({
                "success": False,
                "error": f"エンティティが存在しません: {entity_uuid}"
            }), 404
        ...
        logger.error(f"エンティティ詳細の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_entities_by_type(graph_id: str, entity_type: str):
    ...
    try:
        ...
        logger.error(f"エンティティの取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "プロジェクトにグラフが構築されていません。先に /api/graph/build を呼び出してください"
            }), 400
        ...
        logger.error(f"シミュレーションの作成に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # 检查state.json中的状态（先に読んでプラットフォーム情報を取得）
    state_file = os.path.join(simulation_dir, "state.json")
    if not os.path.exists(state_file):
        return False, {"reason": "state.json が存在しません"}
    ...
    # プラットフォームに応じてプロフィールファイルを追加
    import json as _json
    try:
        with open(state_file, 'r', encoding='utf-8') as _f:
            _state_peek = _json.load(_f)
        if _state_peek.get("enable_reddit", True):
            required_files.append("reddit_profiles.json")
        if _state_peek.get("enable_twitter", True):
            required_files.append("twitter_profiles.csv")
    except Exception:
        # state.json の読み取りに失敗した場合は両方要求
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            return False, {
                "reason": f"状態が準備完了リストにないか、config_generatedがfalseです: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    ...
    返回：
        {
            ...
            "data": {
                ...
                "message": "準備タスクを開始しました | 準備完了の作業があります",
    ...
    try:
        ...
        if not state:
            return jsonify({
                "success": False,
                "error": f"シミュレーションが存在しません: {simulation_id}"
            }), 404
        
        # 強制再生成の確認
        force_regenerate = data.get('force_regenerate', False)
        logger.info(f"/prepare リクエストの処理を開始: simulation_id={simulation_id}, force_regenerate={force_regenerate}")
        
        # 準備完了確認（重複生成防止）
        if not force_regenerate:
            if state.status == SimulationStatus.PREPARING and state.prepare_task_id:
                logger.info(f"シミュレーション {simulation_id} は既存の準備タスクを継続中です: {state.prepare_task_id}")
                return jsonify({
                    "success": True,
                    "data": {
                        ...
                        "status": "preparing",
                        "message": "既存の準備タスクを継続中です",
                        "already_prepared": False,
                        "expected_entities_count": state.entities_count,
                        "entity_types": state.entity_types
                    }
                })
            logger.debug(f"シミュレーション {simulation_id} の準備完了確認...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"確認結果: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"シミュレーション {simulation_id} は準備完了。重複生成をスキップします")
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "ready",
                        "message": "準備完了の作業があります。重複生成は不要です",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"シミュレーション {simulation_id} は未準備。準備タスクを開始します")
        ...
        if not project:
            return jsonify({
                "success": False,
                "error": f"プロジェクトが存在しません: {state.project_id}"
            }), 404
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "シミュレーション要件の記述がプロジェクトにありません (simulation_requirement)"
            }), 400
        ...
        try:
            logger.info(f"同期でエンティティ数を取得中: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # 快速读取实体（不需要边信息，只统计数量）
            ...
            logger.info(f"予想エンティティ数: {filtered_preview.filtered_count}, タイプ: {filtered_preview.entity_types}")
        except Exception as e:
            logger.warning(f"エンティティ数の取得に失敗しました（バックグラウンドタスクで再試行）: {e}")
            # 失败不影响后续流程，后台任务会重新获取
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    # 构建详细进度信息
                    stage_names = {
                        "reading": "グラフエンティティの読み込み",
                        "generating_profiles": "エージェントプロファイル生成",
                        "generating_config": "シミュレーション設定生成",
                        "copying_scripts": "シミュレーションスクリプトの準備"
                    }
                ...
                # 任务完成
                result_state.prepare_task_id = task_id
                ...
                logger.error(f"シミュレーションの準備に失敗しました: {str(e)}")
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "準備タスクを開始しました。/api/simulation/prepare/status で進捗を確認してください",
                "already_prepared": False,
                "expected_entities_count": state.entities_count,
                "entity_types": state.entity_types
            }
        ...
        logger.error(f"準備タスクの起動に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "まだ準備が开始されていません。/api/simulation/prepare を呼び出して準備を始めてください",
                        "already_prepared": False
                    }
                })
            return jsonify({
                "success": False,
                "error": "task_id または simulation_id を指定してください"
            }), 400
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"profiles ファイルの読み込みに失敗しました（書き込み中の可能性）: {e}")
                profiles = []
        ...
        logger.error(f"リアルタイムProfileの取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"config ファイルの読み込みに失敗しました（書き込み中の可能性）: {e}")
                config = None
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "シミュレーション設定が存在しません。先に /prepare を呼び出してください"
            }), 404
        ...
        logger.error(f"設定の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "設定ファイルが存在しません。先に /prepare を呼び出してください"
            }), 404
        ...
        logger.error(f"設定ファイルのダウンロードに失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": f"不明なスクリプト: {script_name}。選択能: {allowed_scripts}"
            }), 400
        ...
        if not os.path.exists(script_path):
            return jsonify({
                "success": False,
                "error": f"スクリプトファイルが存在しません: {script_name}"
            }), 404
        ...
        logger.error(f"スクリプトのダウンロードに失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                if max_rounds <= 0:
                    return jsonify({
                        "success": False,
                        "error": "max_rounds は正の整数である必要があります"
                    }), 400
            except (ValueError, TypeError):
                return jsonify({
                    "success": False,
                    "error": "max_rounds は有効な整数である必要があります"
                }), 400

        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"無効なプラットフォームタイプ: {platform}。オプション: twitter/reddit/parallel"
            }), 400
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            # 强制模式：停止运行中的模拟
                            logger.info(f"強制モード：実行中のシミュレーションを停止します {simulation_id}")
                            try:
                                SimulationRunner.stop_simulation(simulation_id)
                            except Exception as e:
                                logger.warning(f"シミュレーション停止時に警告が発生しました: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                                "error": f"シミュレーションは実行中です。先に /stop API を呼び出して停止するか、force=true を使用して強制的に再開してください。"
                            }), 400

                # 如果是强制模式，清理运行日志
                if force:
                    logger.info(f"強制モード：シミュレーションログをクリーンアップします {simulation_id}")
                    cleanup_result = SimulationRunner.cleanup_simulation_logs(simulation_id)
                    if not cleanup_result.get("success"):
                        logger.warning(f"ログのクリーンアップ時に警告が発生しました: {cleanup_result.get('errors')}")
                    force_restarted = True

                # 进程不存在或已结束，重置状态为 ready
                logger.info(f"シミュレーション {simulation_id} の準備が完了しました。状態を ready にリセットします（元の状態: {state.status.value}）")
                ...
                return jsonify({
                    "success": False,
                    "error": f"シミュレーションの準備ができていません。現在の状態: {state.status.value}。先に /prepare API を呼び出してください。"
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "グラフメモリの更新を有効にするには有効な graph_id が必要です。プロジェクトにグラフが構築されていることを確認してください。"
                }), 400
            
            logger.info(f"グラフメモリの更新を有効化: simulation_id={simulation_id}, graph_id={graph_id}")
        ...
        logger.error(f"シミュレーションの開始に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def stop_simulation():
    ...
    try:
        ...
        logger.error(f"シミュレーションの停止に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"実行状態の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"詳細な状態の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_simulation_actions(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"アクション履歴の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_simulation_timeline(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"タイムラインの取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Agent統計の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "データベースが存在しません。シミュレーションはまだ実行されていない可能性があります",
                }
            })
...
def get_simulation_comments(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"コメントの取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def interview_agent():
    ...
    try:
        ...
        if agent_id is None:
            return jsonify({
                "success": False,
                "error": "agent_id を指定してください"
            }), 400
            
        if not prompt:
            return jsonify({
                "success": False,
                "error": "prompt（インタビュー質問）を指定してください"
            }), 400
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "platform パラメータは 'twitter' または 'reddit' のみを指定できます"
            }), 400
            
        # 環境が実行中か確認
        is_running = SimulationRunner.is_environment_running(simulation_id)
        if not is_running:
            return jsonify({
                "success": False,
                "error": "シミュレーション環境が実行されていないか、終了しています。シミュレーションが完了し、コマンド待機モードに入っていることを確認してください。"
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Interviewの応答待ちタイムアウト: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": "interviews（インタビューリスト）を配列で指定してください"
            }), 400
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"インタビューリストの{i+1}番目に agent_id がありません"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"インタビューリストの{i+1}番目に prompt がありません"
                }), 400
            # 验证每项的platform（如果有）
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"インタビューリストの{i+1}番目のPlatformは 'twitter' または 'reddit' のみ指定できます"
                }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "シミュレーション環境が実行されていないかクローズされています。シミュレーションが完了してコマンド待機モードに入っていることを確認してください。"
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"一括 Interview応答の待機がタイムアウトしました: {str(e)}"
        }), 504

    except Exception as e:
        logger.error(f"一括 Interview の失敗: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def interview_all_agents():
    ...
    try:
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "platform パラメータは 'twitter' または 'reddit' のみ指定できます"
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"全体 Interview応答の待機がタイムアウトしました: {str(e)}"
        }), 504
...
def get_interview_history():
    ...
    try:
        ...
        logger.error(f"Interview履歴の取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
    ...
    返回：
        {
            ...
            "data": {
                ...
                "message": "環境が実行中です。Interviewコマンドを受信できます"
            }
        }
    """
    try:
        ...
        logger.error(f"環境ステータスの取得に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
...
def close_simulation_env():
    ...
    try:
        ...
        logger.error(f"環境終了に失敗しました: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc() if Config.DEBUG else None
