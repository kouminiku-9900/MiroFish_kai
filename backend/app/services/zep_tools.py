...
class SearchResult:
    ...
    def to_text(self) -> str:
        """转换为文本格式，供LLM理解"""
        text_parts = [f"検索クエリ: {self.query}", f"関連情報 {self.total_count} 件を発見しました"]
...
class NodeInfo:
    ...
    def to_text(self) -> str:
        """转换为文本格式"""
        entity_type = next((l for l in self.labels if l not in ["Entity", "Node"]), "不明なタイプ")
        return f"エンティティ: {self.name} (タイプ: {entity_type})\n要約: {self.summary}"
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"- 関係チェーン: {self.total_relationships}本"
        ]
        ...
        if self.entity_insights:
            ...
            for entity in self.entity_insights:
                ...
                if entity.get('summary'):
                    text_parts.append(f"  要約: \"{entity.get('summary')}\"")
...
class AgentInterview:
    """単一Agentのインタビュー結果"""
    agent_name: str
    agent_role: str  # ロールタイプ（例: 学生、教師、メディアなど）
    agent_bio: str  # プロフィール
    question: str  # インタビュー質問
    response: str  # インタビュー回答
    key_quotes: List[str] = field(default_factory=list)  # キー引用
...
class InterviewResult:
    """
    インタビュー結果 (Interview)
    複数のシミュレーションAgentのインタビュー回答を含む
    """
    interview_topic: str  # インタビューテーマ
    interview_questions: List[str]  # インタビュー質問リスト
...
class ZepToolsService:
    ...
    【コア検索ツール - 最適化済み】
    1. insight_forge - 深度インサイト検索（最も強力、自動的にサブ質問を生成、多次元検索）
    2. panorama_search - 広域検索（全体像を取得、過期コンテンツを含む）
    3. quick_search - 簡単検索（高速検索）
    4. interview_agents - 深度インタビュー（シミュレーションAgentをインタビュー、多角的視点を取得）
    
    【基本ツール】
    - search_graph - グラフ意味検索
    - get_all_nodes - グラフの全ノードを取得
    - get_all_edges - グラフの全エッジを取得（時間情報含む）
    - get_node_detail - ノードの詳細情報を取得
    - get_node_edges - ノード関連のエッジを取得
    - get_entities_by_type - タイプ別エンティティを取得
    - get_entity_summary - エンティティの関係サマリーを取得
    """
    ...
    def search_graph(
        ...
        try:
            ...
            logger.warning(f"Zep Search APIが失敗しました。ローカル検索にフォールバックします: {str(e)}")
            # フォールバック: ローカルキーワードマッチング検索を使用
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        Returns:
            节点列表
        """
        logger.info(f"グラフ {graph_id} の全ノードを取得しています...")
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        """
        logger.info(f"グラフ {graph_id} の全エッジを取得しています...")
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            ...
            logger.error(f"ノード詳細の取得に失敗しました: {str(e)}")
            return None
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Returns:
            边列表
        """
        logger.info(f"ノード {node_uuid[:8]}... の関連エッジを取得しています")
        ...
        try:
            ...
            logger.info(f"{len(result)} 件のノード関連エッジが見つかりました")
            return result
            
        except Exception as e:
            logger.warning(f"ノードエッジの取得に失敗しました: {str(e)}")
            return []
    ...
    def get_entities_by_type(
        ...
        Returns:
            符合类型的实体列表
        """
        logger.info(f"タイプ {entity_type} のエンティティを取得しています...")
        ...
        logger.info(f"{len(filtered)} 個の {entity_type} タイプのエンティティが見つかりました")
        return filtered
    ...
    def get_entity_summary(
        ...
        Returns:
            实体摘要信息
        """
        logger.info(f"エンティティ {entity_name} の関係サマリーを取得しています...")
...
返回JSON格式的子问题列表。"""
        ...
        try:
            ...
            logger.warning(f"サブ質問の生成に失敗しました: {str(e)}。デフォルトのサブ質問を使用します")
            # フォールバック：元の質問に基づくバリエーションを返す
            return [
                query,
                f"{query} の主要参加者",
                f"{query} の原因と影響",
    ...
    def interview_agents(
        ...
        if not profiles:
            logger.warning(f"シミュレーション {simulation_id} のプロファイルが見つかりませんでした")
            result.summary = "インタビュー可能なエージェントのプロファイルが見つかりませんでした"
            return result
        
        logger.info(f"初期化済み: {len(profiles)} 個のエージェントプロファイル")
        
        # Step 2: LLMによるインタビュー対象Agent選択（agent_idリストを返す）
        ...
        logger.info(f"{len(selected_agents)} 個のAgentをインタビュー対象に選びました: {selected_indices}")
        ...
        if not result.interview_questions:
            ...
            logger.info(f"{len(result.interview_questions)} 件のインタビュー質問を生成しました")
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # 常にデュアルプラットフォームマーカーを出力
                twitter_text = twitter_response if twitter_response else "（このプラットフォームの回答なし）"
                reddit_text = reddit_response if reddit_response else "（このプラットフォームの回答なし）"
                response_text = f"【Twitterプラットフォームの回答】\n{twitter_text}\n\n【Redditプラットフォームの回答】\n{reddit_text}"
            ...
            # シミュレーション環境が実行されていない
            logger.warning(f"Interview APIの呼び出しに失敗しました（環境が実行されていない可能性）: {e}")
            result.summary = f"インタビュー失敗: {str(e)}。シミュレーション環境が閉じている可能性があります。OASIS環境が実行中であることを確認してください。"
            return result
        except Exception as e:
            logger.error(f"Interview API呼び出し中に例外が発生しました: {e}")
            import traceback
        ...
        logger.info(f"InterviewAgents完了: {result.interviewed_count} 個のAgentをインタビュー（デュアルプラットフォーム）")
        return result
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(reddit_profile_path):
            try:
                ...
                logger.info(f"reddit_profiles.json から {len(profiles)} 個のプロファイルを読み込みました")
                return profiles
            except Exception as e:
                logger.warning(f"reddit_profiles.json の読み込みに失敗しました: {e}")
        ...
        if os.path.exists(twitter_profile_path):
            try:
                ...
                logger.info(f"twitter_profiles.csv から {len(profiles)} 個のプロファイルを読み込みました")
                return profiles
            except Exception as e:
...
请生成采访摘要。"""
        ...
        try:
            ...
            logger.warning(f"インタビューサマリーの生成に失敗しました: {e}")
            # 降级：简单拼接
