...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    固定使用 Gemini。
    支持可选的加速配置，用于并行模拟时使用第二个 Gemini key/model：
    - 通用配置：GOOGLE_AI_API_KEY, LLM_MODEL_NAME
    - 加速配置（可选）：LLM_BOOST_API_KEY, LLM_BOOST_MODEL_NAME
    ...
    if use_boost and has_boost_config:
        ...
        llm_api_key = os.environ.get("GOOGLE_AI_API_KEY", "")
        llm_model = os.environ.get("LLM_MODEL_NAME", "")
        config_label = "[通用LLM]"

    if not llm_model:
        llm_model = config.get("llm_model", "gemini-3.1-flash-lite-preview")

    if not llm_api_key:
        raise ValueError("缺少 API Key 配置，请在项目根目录 .env 文件中设置 GOOGLE_AI_API_KEY")

    os.environ["GOOGLE_API_KEY"] = llm_api_key
    os.environ["GEMINI_API_KEY"] = llm_api_key  # CamelのModelFactoryが参照する環境変数
    os.environ["LLM_PROVIDER"] = "google"

    print(f"{config_label} provider=google, model={llm_model}...")
