    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">{{ $t('main_view.steps.step4') }}</span>
        </div>
...
import { setWorkflowIds } from '../store/workflow'
...
const statusText = computed(() => {
  ...
  return t('main_view.status.generating')
})
...
const loadReportData = async () => {
  try {
    addLog(`レポートデータを読み込み中: ${currentReportId.value}`)
    ...
    if (reportRes.success && reportRes.data) {
      ...
      setWorkflowIds({
        ...
        simulation_id: reportData.simulation_id,
        report_task_id: reportTaskId.value || null
      })
      ...
      addLog(`レポート情報の取得に失敗しました: ${reportRes.error || '不明なエラー'}`)
    }
  } catch (err) {
    addLog(`レポート読込中に例外が発生しました: ${err.message}`)
  }
}
...
watch(() => route.query.taskId, (newTaskId) => {
  reportTaskId.value = newTaskId || null
  if (newTaskId) {
