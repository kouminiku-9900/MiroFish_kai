  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('components.step3.platform1') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('components.step3.round') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('components.step3.elapsed') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('components.step3.actions') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span>
            </span>
          </div>
          <!-- 可用动作提示 -->
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('components.step3.tooltip_title') }}</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('components.step3.action_labels.post') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.like') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.repost') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.quote') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.follow') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.idle') }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('components.step3.platform2') }}</span>
          ...
          <div class="actions-tooltip">
            ...
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('components.step3.action_labels.post') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.comment') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.like') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.dislike') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.search') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.trend') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.follow') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.mute') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.refresh') }}</span>
              <span class="tooltip-action">{{ $t('components.step3.action_labels.idle') }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('components.step3.report_btn_loading') : $t('components.step3.report_btn') }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('components.step3.total_events') }}: <span class="mono">{{ allActions.length }}</span></span>
      ...
      <div class="timeline-feed">
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || 'ユーザー' }}</span>
                    </div>
                ...
                <!-- REPOST: $t('components.step3.action_labels.repost') -->
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('components.step3.timeline.reposted_from', { name: action.action_args?.original_author_name || 'ユーザー' }) }}</span>
                  </div>
                ...
                <!-- LIKE_POST: $t('components.step3.action_labels.like') -->
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    <svg class="icon-small filled" viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                    <span class="like-label">{{ $t('components.step3.timeline.liked_post', { name: action.action_args?.post_author_name || 'ユーザー' }) }}</span>
                  </div>
                ...
                <!-- CREATE_COMMENT: $t('components.step3.action_labels.comment') -->
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('components.step3.timeline.reply_to_post', { id: action.action_args.post_id }) }}</span>
                  </div>
                </template>

                <!-- SEARCH_POSTS: $t('components.step3.action_labels.search') -->
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                    <span class="search-label">{{ $t('components.step3.timeline.search_query') }}:</span>
                    <span class="search-query">"{{ action.action_args?.query || '' }}"</span>
                  </div>
                </template>

                <!-- FOLLOW: $t('components.step3.action_labels.follow') -->
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>
                    <span class="follow-label">{{ $t('components.step3.timeline.followed', { name: action.action_args?.target_user || action.action_args?.user_id || 'ユーザー' }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $t('components.step3.timeline.upvoted_post') : $t('components.step3.timeline.downvoted_post') }}</span>
                  </div>
                ...
                <!-- DO_NOTHING: $t('components.step3.action_labels.idle') -->
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <span class="idle-label">{{ $t('components.step3.timeline.action_skipped') }}</span>
                  </div>
                </template>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>{{ $t('components.step3.waiting') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">{{ $t('components.step3.log_title') }}</span>
...
const formatElapsedTime = (currentRound) => {
  if (!currentRound || currentRound <= 0) return '0時間 0分'
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('エラー: simulationIdが欠落しています')
    return
  }
  ...
  try {
    ...
    addLog('動的グラフ更新モードを有効にしました')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 古いシミュレーションログをクリアし、シミュレーションを再開しました')
      }
      addLog('✓ シミュレーションエンジンが正常に起動しました')
...
const handleStopSimulation = async () => {
  ...
  addLog('シミュレーションを停止しています...')
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog('✓ シミュレーションが停止しました')
      phase.value = 2
      stopPolling()
      setWorkflowStage('simulation_run', {
        status: 'completed',
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      const maxRounds = data.total_rounds || props.maxRounds || 0
      const currentRound = Math.max(data.twitter_current_round || 0, data.reddit_current_round || 0)
      const progress = maxRounds > 0 ? Math.min(100, Math.round((currentRound / maxRounds) * 100)) : 0
      setWorkflowStage('simulation_run', {
        status: 'running',
        progress,
        message: `ラウンド ${currentRound}/${maxRounds || '?'} を実行中`
      })
      
      // 分别检测各平台的轮次变化并输出日志
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[タイムライン] ${data.twitter_current_round}/${data.total_rounds} ラウンド | 経過:${data.twitter_simulated_hours || 0}時間 | 行動:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(`[コミュニティ] ${data.reddit_current_round}/${data.total_rounds} ラウンド | 経過:${data.reddit_simulated_hours || 0}時間 | 行動:${data.reddit_actions_count}`)
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ すべてのプラットフォームのシミュレーション終了を検出しました')
        }
        addLog('✓ シミュレーションが完了しました')
        phase.value = 2
        stopPolling()
        setWorkflowStage('simulation_run', {
          status: 'completed',
    ...
    console.warn('シミュレーション状態の取得に失敗しました:', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('シミュレーション詳細の取得に失敗しました:', err)
  }
}
...
const getActionTypeLabel = (type) => {
  const labels = {
    ...
    'LIKE_POST': t('components.step3.action_labels.like'),
    'CREATE_COMMENT': t('components.step3.action_labels.comment'),
    'LIKE_COMMENT': t('components.step3.action_labels.like'),
    'DO_NOTHING': t('components.step3.action_labels.idle'),
    'FOLLOW': t('components.step3.action_labels.follow'),
    'SEARCH_POSTS': t('components.step3.action_labels.search'),
    'QUOTE_POST': t('components.step3.action_labels.quote'),
    'UPVOTE_POST': t('components.step3.action_labels.upvote'),
    'DOWNVOTE_POST': t('components.step3.action_labels.downvote')
  }
...
const formatActionTime = (timestamp) => {
  if (!timestamp) return ''
  try {
    return new Date(timestamp).toLocaleTimeString('ja-JP', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })
  } catch {
    return ''
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('レポート生成リクエストを送信しました。お待ちください...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('レポート生成を起動しています...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ レポート生成タスクが起動しました: ${reportId}`)
      syncWorkflowFromReport({
        reportId,
        taskId: reportTaskId,
        status: 'running',
      ...
      addLog(`✗ レポート生成機能の起動に失敗: ${res.error || '不明なエラー'}`)
      setWorkflowStage('report_generate', {
        status: 'failed',
        progress: 0,
        message: res.error || 'レポート生成の開始に失敗しました'
      })
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ レポート生成機能の起動異常: ${err.message}`)
    setWorkflowStage('report_generate', {
      status: 'failed',
      progress: 0,
      message: err.message
