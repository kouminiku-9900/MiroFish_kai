  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="$t('graph_panel.refresh')">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">{{ $t('common.refresh') }}</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" :title="$t('graph_panel.maximize')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('graph_panel.live_memory_updating') : $t('graph_panel.updating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graph_panel.finished_hint') }}</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" :title="$t('graph_panel.close_hint')">
        ...
        <div v-if="selectedItem" class="detail-panel">
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            <div class="detail-row">
              <span class="detail-label">{{ $t('graph_panel.name') }}:</span>
              <span class="detail-value">{{ selectedItem.data.name }}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">{{ $t('graph_panel.uuid') }}:</span>
              <span class="detail-value uuid-text">{{ selectedItem.data.uuid }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.created_at">
              <span class="detail-label">{{ $t('graph_panel.created') }}:</span>
              <span class="detail-value">{{ formatDateTime(selectedItem.data.created_at) }}</span>
            </div>
            
            <!-- Properties -->
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              <div class="section-title">{{ $t('graph_panel.properties') }}:</div>
              <div class="properties-list">
                <div v-for="(value, key) in selectedItem.data.attributes" :key="key" class="property-item">
                  <span class="property-key">{{ key }}:</span>
                  <span class="property-value">{{ value || $t('graph_panel.none') }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section" v-if="selectedItem.data.summary">
              <div class="section-title">{{ $t('graph_panel.summary') }}:</div>
              <div class="summary-text">{{ selectedItem.data.summary }}</div>
            </div>
            
            <!-- Labels -->
            <div class="detail-section" v-if="selectedItem.data.labels && selectedItem.data.labels.length > 0">
              <div class="section-title">{{ $t('graph_panel.labels') }}:</div>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                {{ selectedItem.data.source_name }} - {{ $t('graph_panel.self_relations') }}
                <span class="self-loop-count">{{ $t('graph_panel.items', { count: selectedItem.data.selfLoopCount }) }}</span>
              </div>
                ...
                >
                  ...
                  <div class="self-loop-item-content" v-show="expandedSelfLoops.has(loop.uuid || idx)">
                    ...
                    <div class="detail-row" v-if="loop.fact">
                      <span class="detail-label">{{ $t('graph_panel.fact') }}:</span>
                      <span class="detail-value fact-text">{{ loop.fact }}</span>
                    </div>
                    <div class="detail-row" v-if="loop.fact_type">
                      <span class="detail-label">{{ $t('graph_panel.type') }}:</span>
                    ...
                    <div v-if="loop.episodes && loop.episodes.length > 0" class="self-loop-episodes">
                      <span class="detail-label">{{ $t('graph_panel.episodes') }}:</span>
            ...
            <template v-else>
              ...
              <div class="detail-row">
                <span class="detail-label">{{ $t('graph_panel.label') }}:</span>
                <span class="detail-value">{{ selectedItem.data.name || 'RELATED_TO' }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">{{ $t('graph_panel.type') }}:</span>
                <span class="detail-value">{{ selectedItem.data.fact_type || $t('graph_panel.none') }}</span>
              </div>
              ...
              <div class="detail-section" v-if="selectedItem.data.episodes && selectedItem.data.episodes.length > 0">
                <div class="section-title">{{ $t('graph_panel.episodes') }}:</div>
              ...
              <div class="detail-row" v-if="selectedItem.data.valid_at">
                <span class="detail-label">{{ $t('graph_panel.valid_from') }}:</span>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      const nodeName = nodeMap[e.source_node_uuid]?.name || '未設定'
